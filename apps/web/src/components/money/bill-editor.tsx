...
import { useClients, useCreateBill, useUpdateBill, useWorkspace } from "@wedding-yantra/api-client/react";
import { billInput, updateBillInput, type Bill, type BillDraft, type ClientSummary } from "@wedding-yantra/types";
...
function termsFor(issueDate: string, dueDate: string | null, eventDate: string | null): PaymentTermKey {
  if (!dueDate) return "receipt";
  if (eventDate && dueDate === eventDate) return "event";
  const match = PAYMENT_TERMS.find((t) => t.days !== null && addDays(issueDate, t.days) === dueDate);
...
export function BillEditor({ draft, bill, onSaved }: { draft?: BillDraft; bill?: Bill; onSaved: (bill: Bill) => void }) {
  ...
  const businessGst = bill ? bill.chargesGst || !!details.data?.gstin : draft!.chargesGst;
  ...
  // Customer
  const [clientId, setClientId] = useState<string | null>(bill ? bill.clientId : draft!.clientId);
  ...
  const [terms, setTermsKey] = useState<PaymentTermKey>(() => termsFor(start.issueDate, start.dueDate, eventDate));
  const [dueDate, setDueDate] = useState(start.dueDate ?? "");

  // GST
  const [chargesGst, setChargesGst] = useState(bill ? bill.chargesGst : draft!.chargesGst);
  const [pricesIncludeGst, setPricesIncludeGst] = useState(bill?.pricesIncludeGst ?? false);
  ...
  const [lines, setLines] = useState<EditableLine[]>(() =>
    toEditableLines(
      bill?.pricesIncludeGst
        ? bill.items.map((i) => ({ ...i, rate: Math.round(i.rate * (1 + i.taxRate / 100) * 100) / 100 }))
        : start.items,
    ),
  );
  const [discountMode, setDiscountMode] = useState<"amount" | "percent">(bill?.discountPercent ? "percent" : "amount");
  ...
  const prepared = prepareBillLines(numericLines(lines), {
    ...
    discountPercent: discountMode === "percent" && typedDiscount > 0 ? typedDiscount : null,
  });
  const totals = computeBillTotals(prepared.lines, prepared.discount, { chargesGst, interState });
  ...
  function pickTerms(key: PaymentTermKey) {
    ...
    if (t.days !== null) setDueDate(t.days === 0 ? "" : addDays(issueDate, t.days));
  ...
  function toggleGst(on: boolean) {
    ...
    if (on) setLines((ls) => ls.map((l) => (l.taxRate === 0 ? { ...l, taxRate: 18 } : l)));
  }
  ...
  async function submit(e: FormEvent) {
    ...
    const check = bill
      ...
      : validate(billInput, { ...fields, eventId: draft!.eventId, clientId, quoteId: draft!.quoteId, payment });
    if (check.errors) return setErrors(check.errors);
    setErrors({});
    try {
      const saved = bill
        ? await update.mutateAsync(fields)
        : await create.mutateAsync({ ...fields, eventId: draft!.eventId, clientId, quoteId: draft!.quoteId, payment });
      onSaved(saved);
    } catch (err) {
  ...
  const taxLine = [
    totals.tax > 0 && `${interState ? "IGST" : "GST"} ${formatMoney(totals.tax, { paise: true })}`,
  ...
  return (
    <form onSubmit={submit} className="space-y-6 pb-28" noValidate>
      ...
      <Card className="space-y-4 p-5">
        ...
        {!fixedCustomer && !clientId ? (
          <CustomerSearch name={name} onName={setName} onPick={pickClient} error={errors["billTo.name"]} />
        ) : (
        ...
        <TextAreaField
          ...
          hint={chargesGst && totals.total >= 50000 && !address ? "GST invoices of ₹50,000 or more need the customer's address." : undefined}
        />
        {!fixedCustomer && !clientId && name.trim().length >= 2 && (
          <p className="text-sm text-ink-muted">A new customer: they&apos;re added to your clients when you save.</p>
        )}
      </Card>
      ...
      <Card className="space-y-4 p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <TextField label="Invoice date" type="date" value={issueDate} onChange={(e) => changeIssueDate(e.target.value)} error={errors.issueDate} />
          <SelectField label="Payment terms" value={terms} onChange={(e) => pickTerms(e.target.value as PaymentTermKey)}>
            {PAYMENT_TERMS.filter((t) => t.key !== "event" || eventDate).map((t) => (
              <option key={t.key} value={t.key}>
                {t.key === "event" && eventDate ? `Before the event (${formatDate(eventDate, { year: false })})` : t.label}
              </option>
            ))}
          </SelectField>
          {terms !== "receipt" && (
      ...
      {businessGst ? (
        <Card className="space-y-4 p-5">
          <label className="flex cursor-pointer items-start gap-3">
            <input type="checkbox" checked={chargesGst} onChange={(e) => toggleGst(e.target.checked)} className="mt-0.5 size-5 accent-brand" />
            <span>
              <span className="block font-bold">GST invoice</span>
              <span className="block text-sm text-ink-muted">
                {chargesGst ? "GST is added on each line. It prints as a tax invoice." : "No GST on this one. It prints as a plain invoice."}
              </span>
            </span>
          </label>
          {chargesGst && (
            <>
              <label className="flex cursor-pointer items-start gap-3">
                <input type="checkbox" checked={pricesIncludeGst} onChange={(e) => setPricesIncludeGst(e.target.checked)} className="mt-0.5 size-5 accent-brand" />
                <span>
                  <span className="block font-bold">My prices include GST</span>
                  <span className="block text-sm text-ink-muted">A ₹50,000 package stays ₹50,000: the invoice shows ₹42,373 plus ₹7,627 GST.</span>
                </span>
              </label>
              <SelectField
                ...
                hint={interState ? "Another state: IGST instead of CGST and SGST." : "Where the event happens. Your own state: CGST and SGST."}
              >
                {INDIAN_STATES.map((s) => (
      ...
      <LineItemsEditor lines={lines} onChange={setLines} amounts={totals.lines.map((l) => l.amount)} errors={errors} allowGst={chargesGst} detailed />
      ...
      <Card className="space-y-3 p-5">
        <div className="flex flex-wrap items-end gap-3">
          ...
          />
          <div className="mb-0.5 inline-flex rounded-xl bg-cream p-1 text-sm font-bold" role="group" aria-label="Discount in">
            {(["amount", "percent"] as const).map((m) => (
              <button
                ...
                className={cn("h-10 rounded-lg px-4", discountMode === m ? "bg-surface shadow-soft" : "text-ink-muted")}
              >
                {m === "amount" ? "₹" : "%"}
              </button>
            ))}
        ...
        <dl className="space-y-1.5 border-t border-line pt-3 text-sm">
          <Row label="Subtotal" value={formatMoney(totals.subtotal, { paise: true })} />
          {totals.discount > 0 && <Row label={discountMode === "percent" ? `Discount (${typedDiscount}%)` : "Discount"} value={`− ${formatMoney(totals.discount, { paise: true })}`} />}
          {totals.byRate.map((r) =>
            interState ? (
              <Row key={r.rate} label={`IGST ${r.rate}%`} value={formatMoney(r.igst, { paise: true })} />
            ) : (
              <div key={r.rate} className="space-y-1.5">
                <Row label={`CGST ${r.rate / 2}%`} value={formatMoney(r.cgst, { paise: true })} />
                <Row label={`SGST ${r.rate / 2}%`} value={formatMoney(r.sgst, { paise: true })} />
              </div>
            ),
          )}
          {totals.roundOff !== 0 && <Row label="Round off" value={formatMoney(totals.roundOff, { paise: true })} />}
          <div className="flex items-baseline justify-between gap-3 border-t border-line pt-2">
            <dt className="font-bold">Total</dt>
            <dd className="font-display text-xl font-extrabold tabular">{formatMoney(totals.total)}</dd>
          </div>
          {totals.total > 0 && <p className="text-xs text-ink-muted">{rupeesInWords(totals.total)}</p>}
        </dl>
      </Card>
      ...
      {received > 0 && (
        <Notice>
          {formatMoney(received)} already received{bill ? " on this invoice" : " for this booking. It shows on the invoice as paid"}, leaving{" "}
          <span className="font-semibold">{formatMoney(balance)}</span> to collect.
        </Notice>
      )}
          ...
          <label className="flex cursor-pointer items-start gap-3">
            <input
              ...
              onChange={(e) => {
                ...
                if (e.target.checked && !payAmount) setPayAmount(balance > 0 ? String(balance) : "");
              }}
              className="mt-0.5 size-5 accent-brand"
            />
            <span>
              <span className="block font-bold">I&apos;ve received money for this</span>
              <span className="block text-sm text-ink-muted">Full, an advance or a token amount. It&apos;s recorded with the invoice, with a receipt number.</span>
            </span>
          </label>
          {paid && (
            <>
              <div className="flex flex-wrap items-end gap-2">
                ...
                />
                <Button type="button" variant="secondary" onClick={() => setPayAmount(String(balance))}>
                  Full
                </Button>
                <Button type="button" variant="secondary" onClick={() => setPayAmount(String(Math.round(balance / 2)))}>
                  Half
                </Button>
              </div>
              <OptionPills list="payment_method" label="How was it paid?" value={payMethod} onChange={(k) => setPayMethod(k ?? "upi")} error={errors["payment.method"]} />
              <div className="grid gap-4 sm:grid-cols-2">
                <TextField label="Received on" type="date" value={payOn} onChange={(e) => setPayOn(e.target.value)} error={errors["payment.paidOn"]} />
                {payMethod !== "cash" && (
                  <TextField label="Reference (optional)" value={payRef} onChange={(e) => setPayRef(e.target.value)} placeholder="UTR or cheque number" error={errors["payment.reference"]} />
                )}
              </div>
            </>
          )}
      ...
      {(errors._ || errors.clientId || errors.chargesGst) && <Notice tone="danger">{errors._ ?? errors.clientId ?? errors.chargesGst}</Notice>}
      ...
      <div className="pb-safe fixed inset-x-0 bottom-16 z-30 border-t border-line bg-surface/95 px-4 py-3 backdrop-blur lg:bottom-0 lg:left-72">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-4 sm:px-2 lg:px-4">
          ...
          <Button type="submit" loading={create.isPending || update.isPending} className="px-6 sm:px-8">
            {!bill && <IndianRupee className="size-4" />}
            {bill ? "Save invoice" : paid ? "Save and record money" : "Make invoice"}
          </Button>
        </div>
      </div>
    </form>
...
function CustomerSearch({ name, onName, onPick, error }: { name: string; onName: (v: string) => void; onPick: (c: ClientSummary) => void; error?: string }) {
  const { workspace } = useCurrentWorkspace();
  ...
  return (
    <div className="space-y-2">
      <TextField label="Customer name" value={name} onChange={(e) => onName(e.target.value)} error={error} placeholder="Type a name to find a client, or a new one" autoFocus />
      {matches.length > 0 && (
        <ul className="divide-y divide-line overflow-hidden rounded-2xl border border-line">
          {matches.map((c) => (
            <li key={c.id}>
              <button type="button" onClick={() => onPick(c)} className="flex w-full items-center gap-3 px-4 py-2.5 text-left hover:bg-cream">
                <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-cream text-brand-strong">
                  <UserRound className="size-4" />
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate font-semibold">{c.name}</span>
                  {c.phone && <span className="block text-sm text-ink-muted tabular">{c.phone}</span>}
                </span>

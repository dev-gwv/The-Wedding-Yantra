...
const GUIDES: Guide[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "Paid in parts? Tick Paid in parts on the invoice and pick 50/50, 30/40/30 or your own split, in percent or rupees, each with a date. The plan prints on the invoice, money received pays the parts in order, and the client's Pay button and your reminders ask for the part that's due.",
      "List what the client gets (edited photos, the album, reels, hampers) under What they'll get, each with a date; your trade's usual ones are a tap away. On an event's invoice they can join the event's deliverables, and the invoice shows a tick once each is delivered.",
  ...
  },
  {
    ...
    steps: [
      ...
      "Alerts: the bell (top of the screen on a phone) shows new tasks, hand-ins, approvals, send-backs, comments and when someone is stuck. Turn on alerts in Alerts, Settings to get them on your phone even when the app is closed (iPhone: add the app to your Home Screen first).",
      "Everyone who works tasks gets their plan at 8 am, an alert an hour before a task's time, and one the morning after a task is missed. Owners and managers get the team's day at 7 pm. Switch any of them off, and set quiet hours, in Alerts, Settings.",
      "Points: every task someone gives you earns points when it's finished: urgent 12, high 8, normal 5, low 3, plus 2 for finishing by its day and 3 when checked work is approved first time. A task finished every day for a week adds 10, and the owner or a manager can recognise great work for 5. Tasks you give yourself don't count, and each task is paid once.",
      "More, Team scores and points shows the month's board: your points, rank and band, a podium for the top three, and one line on what to do next. Tap a name to see how the points were earned. The owner changes the points, the bands, and whether penalties count (off unless switched on) in More, Points rules.",
      "My day (the sun at the top) is the day in order: events, late, today, waiting on you (sent back, or yours to check), and the week. Start, Done, Hand in, Tomorrow and Stuck are one tap on each task. Done for the day sends what you finished and tomorrow's list on WhatsApp.",

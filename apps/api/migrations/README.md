...
```bash
pnpm --filter @wedding-yantra/api migration:new add_phone_to_bookings
# -> creates migrations/0002_add_phone_to_bookings.sql; write your SQL in it
```
...
3. Statements that can't run in a transaction (e.g. `CREATE INDEX CONCURRENTLY`) aren't supported.
4. A backup is taken automatically right before every deploy (see `docs/DEPLOYMENT.md`).
